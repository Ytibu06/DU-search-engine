...
void WebPage::processDoc(const string &docString, Configuration &config, SplitToolCppJieba &jieba)
{
    ...
    if(doc){
        string _docId = doc->FirstChildElement("docid")->GetText();
        string _docUrl = doc->FirstChildElement("url")->GetText();
        string _docTitle = doc->FirstChildElement("title")->GetText();
        string _docContent = doc->FirstChildElement("content")->GetText();
    }
...
//获取所有词频和topK
void WebPage::calcTopK(vector<string> &keywords, int k, set<string> &stopWordList)
{
    // 清空容器
    _topWords.clear();
    _wordsMap.clear();
